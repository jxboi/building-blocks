...
- **`daily_metrics` — registered state snapshots (the point-in-time trend answer):** current-state tables can't tell you how many *open* tasks or *active* members an org had last March — once state changes, the count is gone. So modules (and product verticals) register **metric providers** in a `MetricRegistry` — key, optional dimension, and a set-based count/sum query (`tasks.open` by status category, `members.active`, `customers.count` by segment, `storage.bytes`) — and the nightly job snapshots every registered metric per org into `daily_metrics` (org_id, metric_key, dimension?, value, date). Registering a metric is one declaration; history accumulates from that day forward (snapshots can't be backfilled — register early, another reason verticals register their metrics at birth). Feeds: org-facing trend charts (dashboard kit reads it through a generic metrics endpoint, permission-scoped), admin console trends, and report exports.
- **What this deliberately is not:** entity-level temporal versioning ("what did this record look like on date X") — that's audit metadata's before/after diffs where evented, and full temporal tables/event sourcing stay rejected. Snapshots answer *aggregate* history; audit answers *individual* history; neither pretends to be the other.
